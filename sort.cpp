...
int replace_values(int y, char**  pointer_to_new_line, char* text);
int sort_strings(int y, char**  pointer_to_new_line, char* text);
...
int sort(char* text, char** pointer_to_new_line)
{
    ...
    for(int i = 1; i < MAXIMUM_NUMBER_OF_COLUMNS; i++)
    {
        ...
        {
            //printf(" %d ", y);
            sort_strings(y, pointer_to_new_line, text);
        }
    }
    return 0;
}
    ...
    while(true)
    {
            ...
            (pointer_to_new_line[y])[line_element + skip_symbols_second_string] != '\n')
        {
            ...
            switch(solution_option)
            {
                case 1 :
                {
                    replace_values(y, pointer_to_new_line, text);
                    return 0;
                }
                case -1 :
                {
